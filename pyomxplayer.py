...
class OMXPlayer(object):
    ...
    _STATUS_REXP = re.compile(r"M: (\d) V:\s*([\d.]+)s\s*\d\w*\s*\d\w* A: -\d")
    _DONE_REXP = re.compile(r"have a nice day.*")

    _LAUNCH_CMD = '/usr/bin/omxplayer -s --no-osd %s %s'
    _PAUSE_CMD = 'p'
    _TOGGLE_SUB_CMD = 's'
    _QUIT_CMD = 'q'
    position = 0
    ...
    def __init__(self, mediafile, args=None, start_playback=False):
        ...
        video_props = self._VIDEOPROP_REXP.match(self._process.readline())
        if video_props:
        ...
        audio_props = self._AUDIOPROP_REXP.match(self._process.readline())
        if audio_props:
    ...
    def _get_position(self):
        while True:
            ...
            try:
                self._process.expect('Cv:')
                line = self._process.before
		line = str(line)
                #print line
		try:
			ind = line.index('A')
